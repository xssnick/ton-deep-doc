...
Полученный айди будет ключем для использования в `dht.findValue`. Повторяем основной процесс из предыдущего раздела, все идентично как и в прошлый раз, но `updateRule` в этот раз будет [dht.updateRule.overlayNodes](#dhtupdateruleoverlaynodes). К полученному списку нод мы можем подкючаться и получать от них данные о блоках и всех событиях шарды.
