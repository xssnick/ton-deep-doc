...
Ячейки делятся на два вида, обычные (Ordinary) и специальные. Большинство ячеек с ккоторыми работает поользователь являются обычными, и просто несут в себе информацию. Но для внутринего функционала сети часто используются специальные ячейки, работа с которыми происходит по немного иному сценарию, зависящему от подтипа специальной ячейки. 
...
### Merkle Proof
Ячейка этого типа является деревом хэшей и несет в себе доказательство принадлежнасти части данных дерева ячеек к полному дереву, при этом проверяющий может не знать полное содержимое дерава, а знать только его хеш. 

Внутри себя ячейка несет корневой хеш объекта, например блока, и дерево хешей его веток, в виде дочерней ячейки. Само внутриннее дерево повторяет по структуре оригинальный объект пруфом которого оно является, но при этом, все ветки не являющиеся частью данных которую нужно доказать и не ведущие к ней - заменены на специальные ячейки типа `PrunnedBranch`, которые вместо данных - хранят их хеш. Таким образом посчитав корневой хеш всего дерева, у нас должен получиться тот самый хеш из данных ячейки типа merkle proof, который должен совпасть с заранее известным нам хешом, например блока. Такой подход позволяет нам доказать, например, наличие транзакции в блоке, при условии что тот кому мы доказывает это - знает хеш блока.

##### Пример пруфа для getAccountState
Объект является пруфом для структуры [ShardStateUnsplit](https://github.com/ton-blockchain/ton/blob/master/crypto/block/block.tlb#L399). Но вместо всего блока - содержит только наш запрошеный аккаунт - `EQCVRJ-RqeZWcDqgTzzcxUIrChFYs0SyKGUvye9kGOuEWndQ`. Все ветки не связаные с ним и не ведущие к нему заменены на `Prunned`, а те что ведут к нему - являются обычными `Ordinary` ячейками и содержат реальные данные блока.
