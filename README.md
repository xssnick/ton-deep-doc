...
4. exit_code:int -> 4 байта, код выхода при выполнении метода, если все успешно то = 0, если нет - равен коду исключения
...
Внутри `result` мы получили `b5ee9c7201010901002f000208000004030102020203030400080ccffcc1020203050600080aabbcc80209040000802007080002bb00000002aa`, это [BoC](#bag-of-cells) содержащий стек с данными. Когда мы десериализуем его - мы получим ячейку:
```
32[00000203] -> {
  8[03] -> {
    0[],
...
Первые 3 байта корневой ячейки `000002` - это глубина стека, тоесть 2. Значит метод вернул нам 2 значения. 
...
```
То увидим что первая ссылка - `rest:^(VmStackList n)` это ячейка следующего значения на стеке, а наше значение `tos:VmStackValue` идет вторым, значит для получения значения нам нужно читать вторую ссылку, тоесть - `32[0CCFFCC1]` это наш первый cell который вернул контракт! 
...
```
Повторяем тот же процесс, 8 бит тип = `03`, тоесть опять cell, вторая ссылка это значение -  `32[0AABBCC8]`, и так как глубина нашего стека равна 2, мы завершаем проход. Итого мы имеем 2 значения возвращенные контрактом - `32[0CCFFCC1]` и `32[0AABBCC8]`. 

Обратите внимание что они в обратном порядке. Точно также нужно передавать и аргументы при вызове функции, в обратном порядке от того что мы видим в коде FunC. 
...
### Bag of Cells
Bag of Cells - формат сериализации ячеек (cells) в массив байт. Описан в виде [TL-B схемы](https://github.com/ton-blockchain/ton/blob/24dc184a2ea67f9c47042b4104bbb4d82289fac1/crypto/tl/boc.tlb#L25).
...
```
Тут мы имеем корневую ячейку размером 1 бит которая имеет 2 ссылки, первая на ячейку размером 24 бита, и вторая ссылка на ячейку размером 7 бит, которая имеет 1 ссылку на ячейку размером 24 бита.
...
Посчитаем дескрипторы для каждой из них. Это 2 байта хранящие флаги, информацию о длине даных и количестве ссылок. Флаги будут опущены в текущем разборе, они почти всегда 0. Первый байт содержит флаги 5 битов флагов и 3 бита количества ссылок. Второй байт - длину полных 4х битных групп (но минимум 1 если не пусто). Получаем:
```
1[8_]      -> 0201 -> 2 ссылки, длина 1 
7[FE]      -> 0101 -> 1 ссылка, длина 1
24[0AAAAA] -> 0006 -> 0 ссылок, длина 6
```
Для данных с непоными 4х битными группами добавляется 1 бит после данных, означающий конечный бит группы, для определения настоящего размера неполных групп. Добавим его:
```
...
Склеим в массив байт и получим:
