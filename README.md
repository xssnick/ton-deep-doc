...
* params:bytes = Наш метод не принимает входных параметров, значит нам нужно передать ему пустой стек (`000000`, 3 байта - стек 0 глубины) сериализованый в [BoC](#bag-of-cells) -> `b5ee9c72010101010005000006000000` -> сериализуем в bytes и получаем `10b5ee9c72410101010005000006000000000000` 0x10 - размер, 3 байта в конце - падинг
