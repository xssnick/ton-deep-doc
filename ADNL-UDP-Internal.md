...
Теперь, когда мы вычислили общий ключ канала, нам нужно сделать из него 2 ключа, один для шифрования исходящих сообщений, другой для дешифрования входящих. Сделать из него 2 ключа довольно просто, второй ключ равен общему ключу записаному в обратном порядке. Пример:
```
Общий ключ : AABB2233
...
04 ed4879a9 000000                                                        -- query (наш dht.getSignedAddressList упакованый в bytes с падингом 3)
...
После сериализации, как и в прошлый раз, мы вычисляем sha256 хеш от контента. Потом шифруем контент пакета с помощью ключа предназначенного для исходящих пакетов канала. [Посчитаем](/ADNL-TCP-Liteserver.md#дополнительные-технические-детали-хендшейка) `pub.aes` ID нашего ключа канала, И собираем наш пакет:
```
bcd1cf47b9e657200ba21d94b822052cf553a548f51f539423c8139a83162180 -- ID нашего ключа канала
6185385aeee5faae7992eb350f26ba253e8c7c5fa1e3e1879d9a0666b9bd6080 -- sha256 хеш контента (до шифрования)
...                                                              -- зашифрованое содержимое пакета
```
Отправляем пакет по UDP и ждем ответ.
