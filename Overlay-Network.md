...
Полученный массив мы оборачиваем в схему `adnl.message.query` и отправляем по ADNL. В ответ мы ждем `overlay.nodes`, это будет список нод этого оверлея к которым мы можем подключиться, и если нужно, повторить этот же запрос уже к ним, пока мы не наберем достаточное количество соединений.

##### Запросим блок у ноды
